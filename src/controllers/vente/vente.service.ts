...
import { generateReference } from "../../shared/utils/generators/reference/reference-generator.service";
...
export class VentesService {
  ...
  async create(createVenteDto: CreateVenteDto) {
    ...
    try {
      ...
      if (createVenteDto.promoCode) {
        promotionCode = await this.promoCodeModel
          .findOne({ code: createVenteDto.promoCode })
          .session(session);
        if (!promotionCode) {
      ...
      for (const item of createVenteDto.items) {
        ...
        if (item.type === "Product") {
          currentItem = await this.productModel
          ...
          currentItem = await this.packModel
            .findById(item.itemId)
            .session(session);
          if (!currentItem) {
            throw new BadRequestException(
              `Pack with id ${item.itemId} not found`
            );
          }
        }
      ...
      const [vente] = await this.venteModel.create(
        [
          {
            createdAt: createVenteDto.createdAt
              ? new Date(createVenteDto.createdAt)
              : new Date(),
            reference,
            client: {
              id: clientId,
            ...
            additionalCharges:
              createVenteDto.additionalCharges?.toFixed(3) || "0.000",
  ...
  async createCommande(commandeVenteDto: CommandeVenteDto) {
    ...
    try {
      ...
      if (commandeVenteDto.promoCode) {
        promotionCode = await this.promoCodeModel
          .findOne({ code: commandeVenteDto.promoCode })
          .session(session);
        if (!promotionCode) {
          throw new BadRequestException("Promotion code not found");
        }
      ...
      for (const item of commandeVenteDto.items) {
        ...
        if (item.type === "Product") {
          ...
          if (!currentItem) {
            throw new BadRequestException(
              `Product with slug ${item.slug} not found`
            );
          }
        } else if (item.type === "Pack") {
          currentItem = await this.packModel
          ...
          if (!currentItem) {
            throw new BadRequestException(
              `Pack with slug ${item.slug} not found`
            );
          }
        }
        ...
        [
          {
            ...
            discount: discount.toFixed(3),
            additionalCharges:
              commandeVenteDto.additionalCharges?.toFixed(3) || "0.000",
            productsDiscount: productDiscount.toFixed(3),
            netAPayer: netAPayer.toFixed(3),
  ...
  async findAll() {
    const ventes = await this.venteModel
      .find()
      .populate("client")
      .populate("promoCode")
  ...
  async findOne(id: string) {
    const vente = await this.venteModel
      .findById(id)
      .populate("items.itemId")
      .populate("client")
      .populate("promoCode");
  ...
  async update(id: string, updateVenteDto: UpdateVenteDto) {
    ...
    try {
      ...
      let livraisonCost = new Decimal(
        updateVenteDto.livraison || existingVente.livraison || 0
      );
      let productDiscount = new Decimal(0);
      let packDiscount = new Decimal(0);
      let discount = new Decimal(0);
      ...
      if (updateVenteDto.promoCode) {
        promotionCode = await this.promoCodeModel
          .findOne({ code: updateVenteDto.promoCode })
          .session(session);
        if (!promotionCode) {
          throw new BadRequestException("Promotion code not found");
        }
      ...
      for (const item of items) {
        ...
        if (!currentItem) {
          throw new BadRequestException(
            `${item.type} with id ${item.itemId} not found`
          );
        }
      ...
      totalTTC = totalHT
        .plus(tva)
        .plus(
          updateVenteDto.additionalCharges ||
            existingVente.additionalCharges ||
            0
        )
        .plus(livraisonCost)
        .plus(advancedInfo.advanced.timber);
      ...
      discount = discount.plus(
        updateVenteDto.additionalDiscount ||
          existingVente.additionalDiscount ||
          0
      );
      ...
      const updateData = {
        ...
        livreur: {
          name:
            updateVenteDto.livreur?.name || existingVente.livreur?.name || "",
          cin: updateVenteDto.livreur?.cin || existingVente.livreur?.cin || "",
          carNumber:
            updateVenteDto.livreur?.carNumber ||
            existingVente.livreur?.carNumber ||
            "",
        },
        items,
        tva: tva.toFixed(3),
        ...
        discount: discount.toFixed(3),
        additionalCharges:
          updateVenteDto.additionalCharges?.toFixed(3) ||
          existingVente.additionalCharges?.toFixed(3) ||
          "0.000",
        productsDiscount: productDiscount.plus(packDiscount).toFixed(3),
        netAPayer: netAPayer.toFixed(3),
        note: updateVenteDto.note || existingVente.note || "",
        modePayment:
          updateVenteDto.modePayment || existingVente.modePayment || "CASH",
      ...
      const updatedVente = await this.venteModel
  ...
  async removeMany(ids: string[]) {
    ...
    const deleteResult = await this.venteModel.deleteMany({
      _id: { $in: ids },
    });
    if (deleteResult.deletedCount === 0) {
      throw new NotFoundException(
        "No matching purchase orders found to delete"
      );
    }
